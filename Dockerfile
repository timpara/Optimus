...
COPY main.py ./
COPY gridplay ./gridplay
# Build + install — this resolves FastAPI, uvicorn, aiosqlite, markdown.
...
ARG VERSION="dev"
LABEL org.opencontainers.image.title="gridplay" \
      org.opencontainers.image.description="Battery Trader Sim — multiplayer energy trading educational game" \
      org.opencontainers.image.source="https://github.com/timpara/gridplay" \
      org.opencontainers.image.url="https://github.com/timpara/gridplay" \
      org.opencontainers.image.documentation="https://github.com/timpara/gridplay#readme" \
...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    GRIDPLAY_DB_PATH=/data/battery_trader.db

# Non-root user.
RUN groupadd --gid 1000 gridplay && \
    useradd --uid 1000 --gid gridplay --create-home gridplay && \
    mkdir -p /data && chown gridplay:gridplay /data

WORKDIR /app

# Bring the pre-built venv and the application files over.
COPY --from=builder /opt/venv /opt/venv
COPY --chown=gridplay:gridplay main.py index.html ./
COPY --chown=gridplay:gridplay docs/legal ./docs/legal
